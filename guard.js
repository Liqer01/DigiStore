// ClosyGuard Enterprise Security Suite v4.5
...
function banIP(ip, durationMinutes = 60, reason = 'Kural ihlali') {
  ...
  console.warn(`[ClosyGuard BAN] IP ${ip} engellendi (${durationMinutes} dk). Neden: ${reason}`);
}
...
function ipBlacklistMiddleware(req, res, next) {
  ...
  if (isBanned(ip)) {
    ...
    return res.status(403).json({
      error: 'Erişiminiz ClosyGuard WAF tarafından güvenlik nedeniyle geçici olarak durduruldu.',
...
function deepWafFilter(req, res, next) {
  ...
  if (queryHit) {
    ...
    return res.status(403).json({ error: 'ClosyGuard WAF: İstek parametrelerinde zararlı kod tespit edildi.' });
  }
  ...
  if (pathHit) {
    ...
    return res.status(403).json({ error: 'ClosyGuard WAF: Şüpheli adres engellendi.' });
  }
  ...
  if (req.body && Object.keys(req.body).length > 0) {
    ...
    if (bodyHit) {
      ...
      return res.status(403).json({ error: 'ClosyGuard WAF: Gönderilen veride zararlı karakterler tespit edildi.' });
    }
  }
...
function trackThreat(ip, score = 1, reason = '') {
  ...
  console.warn(`[ClosyGuard ALERT] IP: ${ip} | Tehdit Puanı: ${current}/5 | Sebep: ${reason}`);
  if (current >= 5) {
...
const globalLimiter = rateLimit({
  ...
  message: {
    error: 'ClosyGuard DDoS Koruması: Çok fazla istek gönderildi. Lütfen 1 dakika sonra tekrar deneyin.',
...
const authLimiter = rateLimit({
  ...
  keyGenerator: (req) => getClientIP(req),
  message: {
    error: 'ClosyGuard: Çok fazla başarısız giriş denemesi. Güvenliğiniz için 15 dakika kilitlendi.',
...
const orderLimiter = rateLimit({
  ...
  keyGenerator: (req) => getClientIP(req),
  message: {
    error: 'ClosyGuard: Sipariş oluşturma limiti aşıldı.',
